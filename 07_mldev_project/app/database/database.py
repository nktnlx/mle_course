...
def init_db():
    ...
    regular_user_exists = session.exec(select(User).where(User.email == 'kate@mail.ru')).first()
    ...
    admin_user_exists = session.exec(select(User).where(User.email == 'alex@mail.ru')).first()
